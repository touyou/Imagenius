...
final class TimeLineViewController: MainViewController {
    ...
    override func load(_ moreflag: Bool) {
        ...
        if !moreflag {
            ...
            twitterManager.loadHomeTimelineTweet(count: 41, success: successHandler, failure: failureHandler)
        } else {
            
            twitterManager.loadHomeTimelineTweet(count: 41, maxID: self.maxId, success: successHandler, failure: failureHandler)
        }
    }
}
